...
if (isset($_GET['sent']) && ($_GET['sent'] == "yes")) {
    ...
    if (preg_match($pattern, $text, $vars) && $CURUSER["class"] >= UC_STAFF) {
	    ...
	    $text = $vars[2] . " - " . $vars[3];
            $text_parsed = format_comment($text);
            sql_query("INSERT INTO shoutbox (userid, avatar, date, text, text_parsed, to_user, staff_shout) VALUES (" . sqlesc($userid) . "," . sqlesc($avatar) . ", $date, " . sqlesc($text) . "," . sqlesc($text_parsed) . "," . sqlesc($to_user) . ", 'no')") or sqlerr(__FILE__, __LINE__);
	    ...
	    $avatar = $CURUSER['avatar'];
            $dailyshouts = 1;
            sql_query("INSERT INTO shoutbox (userid, avatar, date, text, text_parsed, staff_shout) VALUES (" . sqlesc($userid) . "," .sqlesc($avatar)  . ", $date, " . sqlesc($text) . "," . sqlesc($text_parsed) . ", 'no')") or sqlerr(__FILE__, __LINE__);
            sql_query("UPDATE usersachiev SET dailyshouts=dailyshouts+1, weeklyshouts = weeklyshouts+1, monthlyshouts = monthlyshouts+1, totalshouts = totalshouts+1 WHERE id= " . sqlesc($userid)) or sqlerr(__FILE__, __LINE__);
            $mc1->delete_value('shoutbox_');
            //$mc1->delete_value('staff_shoutbox_');
            $HTMLOUT.= "<script type=\"text/javascript\">parent.document.forms[0].shbox_text.value='';</script>";
...
if (($shouts = $mc1->get_value('shoutbox_')) === false) {
    $res = sql_query("SELECT s.id, s.userid, s.date, s.text, s.to_user, s.staff_shout, s.autoshout, u.username, u.avatar, u.pirate, u.perms, u.king, u.class, u.donor, u.warned, u.leechwarn, u.enabled, u.chatpost FROM shoutbox AS s LEFT JOIN users AS u ON s.userid=u.id WHERE s.staff_shout ='no' AND s.autoshout='no' ORDER BY s.id DESC LIMIT 150") or sqlerr(__FILE__, __LINE__);
...
if (count($shouts) > 0) {
    ...
    if ($shouts) {
        ...
        foreach ($shouts as $arr) {
            ...
            $del = ($CURUSER['class'] >= UC_STAFF ? "<a href='./shoutbox.php?del=" . (int)$arr['id'] . "'><img style='max-width:15px;max-height:15px;' src='https://cdn2.iconfinder.com/data/icons/round-interface-1/217/50-512.png'></img></a> " : "");
            $av = $arr['avatar'];
	   // $av	= "<a href=\"javascript:window.top.SmileIT('[b][i]@@&nbsp;[color=#" . get_user_class_color($arr['class']) . "]" . htmlsafechars($arr['username'])) . "[/color]&nbsp;@@[/i][/b]','shbox','shbox_text')\"><img height='10' src='"{$arr['avatar']}"' title='Reply' alt='Reply' style='border:none;' /></a>" : "");    
	    $delall = ($CURUSER['class'] == UC_MAX ? "<a href='./shoutbox.php?delall' onclick=\"confirm_delete(); return false;\"><i class='fas fa-times' style='color:red'></i></a> " : "");
            //$delall
            $pm = ($CURUSER['id'] != $arr['userid'] ? "<span class='date' style=\"color:$dtcolor\"><a target='_blank' href='./pm_system.php?action=send_message&amp;receiver=" . (int)$arr['userid'] . "'><img style='max-height:15px;max-width:15px;' src='{$INSTALLER09['pic_base_url']}pme.png'></img></a></span>\n" : "");
            $date = get_date($arr["date"], 'D', 'J');
            ...
            $user_stuff['avatar'] = $arr['avatar'];
	        //$ts = date_format($date,"D, j");
            $HTMLOUT.= "<tr style='background-color:transparent;'>
				<td class='padding-0'>
<!--<span data-tooltip class='right' tabindex='1' title='". htmlsafechars($date)."'><i class='far fa-clock'></i></span>-->
					<img style='height:35px;max-width:35px;' src='{$av}' ></img> " . format_username($user_stuff, true) . "<span style='color:white;'>$date</span>$del$edit$pm$reply$private&nbsp;<span style='color:white;'>" . format_comment($arr["text"]) . "
        ...
        if (empty($shouts)) $HTMLOUT.= "<tr style='background-color:transparent!important;'><td><span class='size1' style='color:$fontcolor;'>{$lang['shoutbox_no_shouts']}</span></td></tr>\n";
        $HTMLOUT.= "</table>";
    }
}
$HTMLOUT.= "</body></html>";
